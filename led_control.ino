 ...
 * modified by gnk https://github.com/gnkarn/MavLink_FrSkySPort.git
 * incorpora varios efectos ( colocar fuente de los efectos de fastled
...
CRGB Vir_led[NUM_LEDS]     ;                      // Initializxe our array
#define BRIGHTNESS          96

CRGB leds[NUM_ARMS][NUM_LEDS_PER_STRIP]; // matriz virtual hacer que sea una tira continua alrededor
...
SimplePatternList gPatterns = {brazoSenoidal,circling,sinelon, rainbow, rainbowWithGlitter, confetti, juggle, bpm,  sinelon2 };
...
#define SIDE  5
#define FLASH  6
int  LED_DEF[NUM_ARMS][NUM_LEDS_PER_STRIP] = {{ARMED,ARMED,ARMED,ARMED,ARMED,ARMED,ARMED,ARMED,ARMED,ARMED,ARMED,ARMED,ARMED,ARMED,ARMED,ARMED,     GPS,   GPS,   GPS,   GPS,   GPS,       ARMED,ARMED,  ARMED, ARMED, ARMED, ARMED, ARMED, ARMED, ARMED, ARMED, ARMED, ARMED, ARMED, ARMED, ARMED,ARMED,  SIDE,SIDE,SIDE,SIDE,SIDE,SIDE,SIDE,SIDE,SIDE,SIDE,SIDE,SIDE, SIDE },
...
void Teensy_LED_Init() {   // comment in SIMULATION_MODE
 ...
 //FastLED.addLeds<WS2811, 6,RGBORDER >(leds[0], NUM_LEDS_PER_STRIP);  //Right front arm
  FastLED.addLeds<WS2812B, 8,RGBORDER>(leds[0], NUM_LEDS_PER_STRIP);  //Left front arm
  FastLED.addLeds<WS2812B, 7,RGBORDER>(leds[1], NUM_LEDS_PER_STRIP);  //Left front arm
  FastLED.addLeds<WS2812B, 6,RGBORDER>(leds[2], NUM_LEDS_PER_STRIP);  //Left front arm
	...
	*/
	...
	if (STATUS == 1) {
		...
		case 1: //blink orange
			...
			break;
		case 2: // blue
			COLOR = CHSV(200, 255, 128 * dim);
			FREQ = 500;
			break;
		case 3: // const blue
			COLOR = CHSV(200, 255, 255 * dim);
			FREQ = 0;
			state_GPS = 0;
			break;
    case 4: // const blue
      COLOR = CHSV(220, 255, 255 * dim);
      FREQ = 0;
      state_GPS = 0;
      break;
		default: // off
...
void HeliLed (int bpm , int hue,float dim  ){
 uint8_t Brazo0= beatsin8(bpm , 0, dim ,0,0);
  uint8_t Brazo1= beatsin8(bpm , 0, dim ,0,83);
   uint8_t Brazo2= beatsin8(bpm , 0, dim ,0,166);
